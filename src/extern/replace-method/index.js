...
function replacer(ast) {
    ...
    let replace = (methodPath, updater) => {
        ...
        function single(node) {
            if (node.type !== 'CallExpression' && node.type !== 'Identifier')
                return;
            if (node.type === 'CallExpression' && methodPath[0] !== node.callee.name)
                return;
            if (node.type === 'Identifier' && methodPath[0] !== node.name)
                return;
        ...
        function nested(node) {
            ...
            while (c && c.type === 'MemberExpression') {
                ...
                if (c.computed)
                    return;
                if (methodPath[i] !== c.property.name)
                    return;
                c = c.object;
                i = i - 1;
            }
