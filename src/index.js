...
const convertToIntegerKey_1 = require("./utils/convertToIntegerKey");
const browserify_1 = require("./decoders/browserify");
const webpack_1 = require("./decoders/webpack");
...
if (config.knownPaths) {
    config.knownPaths = convertToIntegerKey_1.convertToIntegerKey(config.knownPaths);
}
else {
...
if (config.type === 'browserify') {
    ...
    modules = browserify_1.browserifyDecoder(iifeModules);
}
else {
    modules = webpack_1.webpackDecoder(iifeModules, config.knownPaths);
}
// ------------------------------------------------------------------------------
...
modules = transformRequires_1.transformRequires(modules, config.knownPaths, config.entryPoint, config.type, config.replaceRequires);
// ------------------------------------------------------------------------------
...
const files = lookupTable_1.lookupTableResolver(modules, config.knownPaths, config.entryPoint, config.type, outputLocation);
// ------------------------------------------------------------------------------
// Finally, write the bundle to disk in the specified output location.
// ------------------------------------------------------------------------------
console.log('* Writing to disk...');
const writeToDisk_1 = require("./writeToDisk");
writeToDisk_1.writeToDisk(files);
