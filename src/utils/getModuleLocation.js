...
const getModulePath_1 = require("./getModulePath");
const path = require("path");
// Given a module, return it's location on disk.
function getModuleLocation(modules, mod, knownPaths = {}, pathPrefix = 'dist' + path.sep, appendTrailingIndexFilesToNodeModules = false, entryPointModuleId = 1) {
    let moduleHierarchy;
    let modulePaths = [];
    // Assemble a tree of modules starting at the entry point.
    let tree = getModulePath_1.default(modules, entryPointModuleId);
    ...
    if (mod.lookup) {
        ...
        let { completeEvents, incompleteEvents } = getModulePath_1.getAllPathsToModule(tree, mod.id, knownPaths);
...
exports.default = getModuleLocation;
...
if (require.main === module) {
    let modules = [
        ...
        { id: 6, code: null, lookup: { './lib/rnd': 8 } },
        { id: 7, code: null, lookup: { './lib/rnd': 8 } },
        { id: 8, code: null, lookup: {} },
    ];
    let output = getModuleLocation(modules, modules.find(i => i.id === 8), { 1: './hello/world' });
    console.log(output);
}
//# sourceMappingURL=getModuleLocation.js.map