...
const replace_method_1 = require("./extern/replace-method");
const path = require("path");
const getModuleLocation_1 = require("./utils/getModuleLocation");
    ...
    return modules.map(mod => {
        ...
        if (mod.code && mod.code.params && mod.code.params.length > 0) {
            ...
            if (requireFunctionIdentifier) {
                replace_method_1.replacer(mod.code)(requireFunctionIdentifier.name, // the function that require is in within the code.
                    // the function that require is in within the code.
                    node => {
                    switch (node.type) {
                        case 'CallExpression':
                            ...
                            if (node.arguments[0].type === 'Literal') {
                                ...
                                // This module's path
                                path.dirname(getModuleLocation_1.default(modules, mod, knownPaths, path.sep, /* appendTrailingIndexFilesToNodeModules */ true, entryPointModuleId)), 
                                // The module to import relative to the current module
                                getModuleLocation_1.default(modules, moduleToRequire, knownPaths, path.sep, /* appendTrailingIndexFilesToNodeModules */ false, entryPointModuleId));
                                // If the module path references a node_module, then remove the node_modules prefix
                                if (moduleLocation.indexOf('node_modules' + path.sep) !== -1) {
                                    moduleLocation = moduleLocation.match(new RegExp("node_modules\\" + path.sep + "(.+)$", "g"))[1];
                                }
                ...
                if (replaceRequires === 'variable' && requireFunctionIdentifier.name !== 'require' &&
                    ...
                    mod.code.body.body.unshift(buildVariableAssignment(requireFunctionIdentifier, { type: 'Identifier', name: 'require' }));
                }
            }
            ...
            if (moduleIdentifier && moduleIdentifier.name !== 'module') {
                if (replaceRequires === 'inline') {
                    ...
                    replace_method_1.replacer(mod.code)(moduleIdentifier.name, // the function that require is in within the code.
                        // the function that require is in within the code.
                        node => {
                        node.name = 'module';
                        return node;
                    ...
                    mod.code.body.body.unshift(buildVariableAssignment(moduleIdentifier, { type: 'Identifier', name: 'module' }));
                }
            }
            ...
            if (exportsIdentifier && exportsIdentifier.name !== 'exports') {
                if (replaceRequires === 'inline') {
                    ...
                    replace_method_1.replacer(mod.code)(exportsIdentifier.name, // the function that require is in within the code.
                        // the function that require is in within the code.
                        node => {
                        node.name = 'exports';
                        return node;
                    ...
                    mod.code.body.body.unshift(buildVariableAssignment(exportsIdentifier, { type: 'Identifier', name: 'exports' }));
                }
            }
        }
        else {
...
//# sourceMappingURL=transformRequires.js.map